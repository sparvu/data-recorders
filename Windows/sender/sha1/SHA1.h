...
namespace umtl
{
	...
	public:
		...
		sha1( std::string const && str );
		sha1( char const * str );
		sha1( std::wstring const & str );
		sha1( std::wstring const && str );
		sha1( wchar_t const * str );
		~sha1();
		...
		sha1 & operator+=( std::string const & str );
		sha1 & operator+=( std::string const && str );
		sha1 & operator+=( std::wstring const & str );
		sha1 & operator+=( std::wstring const && str );
		sha1 & operator+=( const char * str );
		sha1 & operator+=( const wchar_t * str );

		friend sha1 & operator+( sha1 & inst, std::string const & str );
		friend sha1 & operator+( sha1 & inst, std::string const && str );
		friend sha1 & operator+( sha1 & inst, std::wstring const & str );
		friend sha1 & operator+( sha1 & inst, std::wstring const && str );
		friend sha1 & operator+( sha1 & inst, char const * str );
		friend sha1 & operator+( sha1 & inst, wchar_t const * str );

		sha1 & operator[]( std::basic_string< TCHAR > const & filename );
		sha1 & operator[]( std::basic_string< TCHAR > const && filename );
		sha1 & operator[]( const TCHAR * filename );
		...
		sha1 & operator=( std::wstring const && str );
		sha1 & operator=( const char * str );
		sha1 & operator=( wchar_t const * str );

		const TCHAR * c_str();
		const char * c_buf();

		friend std::wistream & operator >> ( std::wistream & istream, sha1 & inst );
		friend std::istream & operator >> ( std::istream & istream, sha1 & inst );
...
#ifdef SHA1_UTILITY_FUNCTIONS
		bool ReportHash(TCHAR* tszReport, size_t len, REPORT_TYPE rtReportType = REPORT_HEX_SHORT) const;
#endif

#ifdef SHA1_STL_FUNCTIONS
		bool ReportHashStl(std::basic_string<TCHAR>& strOut, REPORT_TYPE rtReportType =
			REPORT_HEX_SHORT) const;
#endif
	...
	private:
		...
		std::basic_string< TCHAR > hash_str_;
		bool final_;
	};
}
