...
Since 2009, the main architecture of the data recorders was simple designed as a monolithic stack, based on C and Perl5. Perl was a good fit for the entire string processing part, combined with the speed of C language. However over time, we have received lots of feedback from different users and we have discovered several problems with the default current C/Perl implementation:

* runtime errors, difficult sometimes to track down and debug
* high memory usage fetching performance data from +1000 data surces
...
To address all these issues, after 2020, I have started to re-think the recorders based on a different design and architecture, which will allow us to be able to run and operate different recorders, as efficient as possible without the need of an entire runtime engine and avoid runtime errors as much as possible. Same time, fast text processing and support for different network protocols, would remain paramount on the new design. 
