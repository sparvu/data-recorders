...
To really understand how our world works, we need to have the **right data** to analyze. Like understand why airplanes sometimes crash, we look and analyze the data from Flight Data Recorder (FDR). Same for trains or vessels and cars. For Information Technology (IT) too, from computer systems, enterprise applications to other industrial workloads or systems. But in other industries there strict regulations, rules followed and checked by different agencies to ensure things are properly defined and implemented. there is no such thing in IT. So how could we know what we need to capture from all these systems or applications. What metrics? How often? In what format? _See IT world vs others._ (separate link)
